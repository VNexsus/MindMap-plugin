/*
 Adaptation to initial software:
 
 (c) Vitaly Shaev 2021-2022

 */

/*!
 * ====================================================
 * kityminder-editor - v1.0.67 - 2019-02-15
...
(function () {
	...
	Asc.plugin.init = function(data)
	{
		...
		angular.module('kityminderPane', ['kityminderEditor'])
			...
			.controller('MainController', function($scope) {
				$scope.initEditor = function(editor, minder) {
					...
					initData(minder,editor);
					
				};
			});
		function initData(minder,editor){
			minder.importData('json', data);
		}
		if(data)
			window.minder.importData('json', data);

		window.Asc.plugin.resizeWindow(1200, 1000, 800, 600, 0, 0);
		$(window.parent.document).find(".asc-window.modal").find(".header").on('dblclick', toggleSize);
	};
	...
	Asc.plugin.button = function(id)
	{
  		if(id==0){
			...
			minder.exportData('json').then(function(data){
				var json_data = data;
				minder.exportData('png').then(function(data){
					var img_data = data;
					var box = minder.getRenderContainer().getBoundaryBox();
					...
					setTimeout(function() {
						var params = {
							guid : info.guid,
							widthPix : box.width,
							heightPix : box.height,
							width : (box.width + 41) / info.mmToPx,
							height : (box.height + 41) / info.mmToPx,
							imgSrc : img_data,
							data : json_data,
            ...
            function updatePosition() {
                ...
                if (!planed.timer) {
                    planed.timer = setTimeout(function() {
                        ...
                        receiverElement.style.left = Math.round(box.x - 7) + "px";
                        receiverElement.style.top = (debug.flaged ? Math.round(box.bottom + 30) : Math.round(box.y - 7)) + "px";
                        receiverElement.focus();
                        planed.timer = 0;
                    });
                }
...
_p[14] = {
    value: function(require, exports, module) {
        ...
        function MinderRuntime() {
            ...
            minder.execCommand("text", "Центральная тема");
            // 导出给其它 Runtime 使用
            this.minder = minder;
        }
...
_p[15] = {
    value: function(require, exports, module) {
        function NodeRuntime() {
            ...
            var fsm = this.fsm;
            var main = hotbox.state("main");
            var buttons = [ "Повысить:Alt+Up:ArrangeUp", "Создать ниже:Tab|Insert:AppendChildNode", "Создать рядом:Enter:AppendSiblingNode", "Понизить:Alt+Down:ArrangeDown", "Удалить:Delete|Backspace:RemoveNode", "Создать выше:Shift+Tab|Shift+Insert:AppendParentNode" ];
            var AppendLock = 0;
                ...
                main.button({
                    ...
                    action: function() {
                        if (command.indexOf("Append") === 0) {
                            AppendLock++;
                            minder.execCommand(command, "Новая тема");
...
angular.module('kityminderEditor').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('ui/directive/noteEditor/noteEditor.html',
    "<div class=\"panel panel-default\" ng-init=\"noteEditorOpen = false\" ng-show=\"noteEditorOpen\"><div class=\"panel-heading\"><h3 class=\"panel-title\">Note</h3><span>（<a class=\"help\" href=\"https://www.zybuluo.com/techird/note/46064\" target=\"_blank\">See GFM help</a>）</span> <i class=\"close-note-editor glyphicon glyphicon-remove\" ng-click=\"closeNoteEditor()\"></i></div><div class=\"panel-body\"><div ng-show=\"noteEnabled\" ui-codemirror=\"{ onLoad: codemirrorLoaded }\" ng-model=\"noteContent\" ui-codemirror-opts=\"{\n" +
    "                gfm: true,\n" +
    "                breaks: true,\n" +
    "                lineWrapping : true,\n" +
    "                mode: 'gfm',\n" +
...
<div class=\"btn-group-vertical\" dropdown is-open=\"isopen\"><button type=\"button\" class=\"btn btn-default search\" title=\"{{ 'search' | lang:'ui' }}\" ng-class=\"{'active': isopen}\" ng-click=\"enterSearch()\"></button> <button type=\"button\" class=\"btn btn-default search-caption dropdown-toggle\" ng-click=\"enterSearch()\" title=\"{{ 'search' | lang:'ui' }}\"><span class=\"caption\">{{ 'search' | lang:'ui' }}</span> <span class=\"sr-only\">{{ 'search' | lang:'ui' }}</span></button></div>*/
  ...
  $templateCache.put('ui/directive/searchBtn/searchBtn.html',
    "<div class=\"btn-group-vertical\" dropdown is-open=\"isopen\"><a class=\"btn-wrap\" ng-click=\"enterSearch()\"><span class=\"btn-icon search\"></span><span class=\"caption\">{{ 'search' | lang:'ui' }}</span><span class=\"sr-only\">{{ 'search' | lang:'ui' }}</span></a></div>"
  );
  ...
  $templateCache.put('ui/dialog/image/image.tpl.html',
    "<div class=\"modal-header\"><h3 class=\"modal-title\">Insert image</h3></div><div class=\"modal-body\"><tabset><tab heading=\"URL\"  active=\"true\"><form><div class=\"form-group\" ng-class=\"{true: 'has-success', false: 'has-error'}[urlPassed]\"><label for=\"image-url\">URL：</label><input type=\"text\" class=\"form-control\" ng-model=\"data.url\" ng-blur=\"urlPassed = data.R_URL.test(data.url)\" ng-focus=\"this.value = data.url\" ng-keydown=\"shortCut($event)\" id=\"image-url\" placeholder=\"http(s)://\"></div><div class=\"form-group\" ng-class=\"{'has-success' : titlePassed}\"><label for=\"image-title\">Image title：</label><input type=\"text\" class=\"form-control\" ng-model=\"data.title\" ng-blur=\"titlePassed = true\" id=\"image-title\" placeholder=\"Image title\"></div><div class=\"form-group\"><label for=\"image-preview\">Preview：</label><img class=\"image-preview\" id=\"image-preview\" ng-src=\"{{ data.url }}\" alt=\"{{ data.title }}\"></div></form></tab><tab heading=\"Upload\"><form><div class=\"form-group\"><input type=\"file\" name=\"upload-image\" id=\"upload-image\" class=\"upload-image\" accept=\".jpg,.JPG,jpeg,JPEG,.png,.PNG,.gif,.GIF\" onchange=\"angular.element(this).scope().uploadImage()\"><label for=\"upload-image\" class=\"btn btn-primary\"><span>Select file&hellip;</span></label></div><div class=\"form-group\" ng-class=\"{'has-success' : titlePassed}\"><label for=\"image-title\">Image title：</label><input type=\"text\" class=\"form-control\" ng-model=\"data.title\" ng-blur=\"titlePassed = true\" id=\"image-title\" placeholder=\"\"></div><div class=\"form-group\"><label for=\"image-preview\">Preview：</label><img class=\"image-preview\" id=\"image-preview\" ng-src=\"{{ data.url }}\" title=\"{{ data.title }}\" alt=\"{{ data.title }}\"></div></form></tab></tabset></div><div class=\"modal-footer\"><button class=\"btn btn-primary\" ng-click=\"ok()\">OK</button> <button class=\"btn btn-warning\" ng-click=\"cancel()\">Cancel</button></div>"
  );
		...
		return {
			...
			'en-us': {
				...
				'theme': {
					...
					'fresh-purple': 'Purple',
					'fresh-pink': 'Pink',
					'fresh-red-compat': 'Red (compact)',
					'fresh-soil-compat': 'Soil (compact)',
					'fresh-green-compat': 'Green (compact)',
					'fresh-blue-compat': 'Blue (compact)',
					'fresh-purple-compat': 'Purple (compact)',
					'fresh-pink-compat': 'Pink (compact)',
					'tianpan':'Snake',
					'tianpan-compact': 'Snake (compact)'
				},
				...
				'ui': {
					...
					'command': {
						'appendsiblingnode': 'Add sibling',
                        'appendparentnode': 'Add parent',
						'appendchildnode': 'Add child',
						'removenode': 'Remove',
						'editnode': 'Edit',
						'arrangeup': 'Move up',
						'arrangedown': 'Move down',
						'resetlayout': 'Reset layout',
						'expandtoleaf': 'Expand to',
						'expandtolevel1': 'level 1',
						'expandtolevel2': 'level 2',
						'expandtolevel3': 'level 3',
						'expandtolevel4': 'level 4',
						'expandtolevel5': 'level 5',
						'expandtolevel6': 'level 6',
					...
					'menu': {
						...
						'savetab': 'Save tab',
						'sharetab': 'Share tab',
						'preferencetab': 'Preference tab',
						'helptab': 'Helpt ab',
						'feedbacktab': 'Feedback tab',
						'recenttab': 'Recent tab',
						'netdisktab': 'Netdisk tab',
						'localtab': 'Local tab',
						'drafttab': 'Draft tab',
						'downloadtab': 'Download tab',
						'createsharetab': 'Create share tab',
						'managesharetab': 'Manage share tab',
					...
					'acceptfile': 'Accepts： {0}',
					'dropfile': 'Drop file',
					'unsupportedfile': 'Unsupported file',
					'untitleddoc': 'Untitled document',
					'overrideconfirm': 'Confirm overwrite {0}',
					'checklogin': 'Check login...',
					'loggingin': 'Login in progress...',
					'recent': 'Recent',
					'clearrecent': 'Clear recent',
					'clearrecentconfirm': 'Confirm recent clearance',
					'cleardraft': 'Clear draft',
					'cleardraftconfirm': 'Confirm draft clearance',
					...
					'selectrevert': 'Revert selection',
					'selectsiblings': 'Select bindings',
			...
			'ru-ru': {
				'template': {
					'default': 'Стандартный',
					'tianpan': 'Спираль',
					'structure': 'Организационная схема',
					'filetree': 'Деревовидная схема',
					'right': 'Логическая схема',
					'fish-bone': 'Диаграмма Исикавы'
				},
				'theme': {
					'classic': 'Классическая',
					'classic-compact': 'Классическая (compact)',
					'snow': 'Snow',
					'snow-compact': 'Snow (compact)',
					'fish': 'Fish',
					'wire': 'Схема',
					'fresh-red': 'Красный',
					'fresh-soil': 'Коричневый',
					'fresh-green': 'Зеленый',
					'fresh-blue': 'Голубой',
					'fresh-purple': 'Фиолетовый',
					'fresh-pink': 'Розовый',
					'fresh-red-compat': 'Красный (compact)',
					'fresh-soil-compat': 'Коричневый (compact)',
					'fresh-green-compat': 'Зереный (compact)',
					'fresh-blue-compat': 'Голубой (compact)',
					'fresh-purple-compat': 'Фиолетовый (compact)',
					'fresh-pink-compat': 'Розовый (compact)',
					'tianpan':'Круги',
					'tianpan-compact': 'Круги (compact)'
				},
				...
				'ui': {
					...
					'command': {
						'appendsiblingnode': 'Узел рядом',
                        'appendparentnode': 'Узел до',
						'appendchildnode': 'Узел после',
						'removenode': 'Удалить',
						'editnode': 'Изменить',
						'arrangeup': 'Повысить',
						'arrangedown': 'Понизить',
						'resetlayout': 'Сбросить',
						'expandtoleaf': 'Развернуть',
						'expandtolevel1': 'до уровня 1',
						'expandtolevel2': 'до уровня 2',
						'expandtolevel3': 'до уровня 3',
						'expandtolevel4': 'до уровня 4',
						'expandtolevel5': 'до уровня 5',
						'expandtolevel6': 'до уровня 6',
					...
					'appendsiblingnode': 'Добавить рядом',
					'appendchildnode': 'Добавить после',
					'arrangeup': 'Повысить',
					'arrangedown': 'Понизить',
					'editnode': 'Изменить',
					'removenode': 'Удалить',
					...
					'selectrevert': 'Обратить выделение',
					'selectsiblings': 'Соседние элементы',
					'selectlevel': 'Элементы этого уровня',
					'selectpath': 'Вышестоящие элементы',
					'selecttree': 'Нижестоящие элементы'
				},
				'popupcolor': {
					'clearColor': 'clearColor',
					'standardColor': 'standardColor',
